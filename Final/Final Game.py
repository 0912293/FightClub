...
def createBoard():
    ...
    for i in range(tiles):
        for j in range(tiles):
            if numberOfBoardRefreshes == 0 and (i == 0 or j == 0 or j == tiles-1 or i == tiles-1):
                pygame.display.flip()
...
def menu():
    ...
    if ingame:
        if save_button.collidepoint(mpos) & b1==1:
            ...
            saveFeedback_img = pygame.transform.scale(pygame.image.load('b_saved.png'), (screenX//3, 150))
            screen.blit(saveFeedback_img,(screenX//3, screenY//3))
            pygame.display.flip()
            sleep(2)
            print("Saved!")
...
def playerMove(s, forward):
    if players[s].Position + forward > len(tilelist)-1:
        ...
        players[s].Position = remainder-1
    else:
...
def fight(player, defender, tile, dicelist):
    ...
    for i in range(2):
        ...
        healthStat = font.render('Health: {0}' .format(players[p].Health), True, (0,0,0))
        staminaStat = font.render('Stamina: {0}' .format(players[p].Stamina), True, (0,0,0))
        screen.blit(healthStat, (screenX//2*i+(screenX//tiles)+10, screenY//tiles+10))
        screen.blit(staminaStat, (screenX//2*i+(screenX//tiles)+10, screenY//tiles+40))
...
def checkHealth():
    ...
    if activePlayers == 1:
        ...
        while True:
            ...
            exit_button = Rect(screenX//2-(screenX//tiles*1.5), screenY-(screenY//tiles*3), screenX//tiles*3, screenY//tiles)
